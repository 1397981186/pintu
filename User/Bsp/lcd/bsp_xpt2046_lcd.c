...
void judge(char* map[][3]);
void display(char* map[][3]);
void MoveNumber(char* map[][3],strType_XPT2046_Coordinate * touch);
int judgedir(strType_XPT2046_Coordinate * touch);
void IsWin(char* map[][3]);
char* map[3][3]={{"1","2","3"},{"4","5","6"},{"7","8"," "}};
int Crx=2;
int Cry=2;
int flag=0;
int touchcount=0;
...
void XPT2046_TouchDown(strType_XPT2046_Coordinate * touch)
{
	...
	if(touch->pre_x == -1 && touch->pre_x == -1)
		return;
	MoveNumber(map,touch);
	display(map);
	unsigned char * title5 ="√";
	unsigned char * title6 ="×";
	unsigned char * last = "9";
	IsWin(map);
	//还未成功
	if (flag==1)
	{
		ILI9341_DisplayStringEx(20,180,40,40,title5,0);
		ILI9341_DisplayStringEx(2*80+80,2*80,80,80,last,0);
		rt_thread_delay(500);
	}
	//成功还原
	else if (flag==0)
	{
		ILI9341_DisplayStringEx(20,180,40,40,title6,0);
	}
}
...
int judgedir(strType_XPT2046_Coordinate *touch)
{
	if ((touch->x>Crx*80+80)&&(touch->x<(Crx+1)*80+80)&&(touch->y>(Cry+1)*80)&&(touch->y<(Cry+2)*80))
		return 1;
	else if ((touch->x>Crx*80+80)&&(touch->x<(Crx+1)*80+80)&&(touch->y>(Cry-1)*80)&&(touch->y<(Cry)*80))
		return 2;
	else if ((touch->x>(Crx+1)*80+80)&&(touch->x<(Crx+2)*80+80)&&(touch->y>(Cry)*80)&&(touch->y<(Cry+1)*80))
		return 3;
	else if ((touch->x>(Crx-1)*80+80)&&(touch->x<(Crx)*80+80)&&(touch->y>(Cry)*80)&&(touch->y<(Cry+1)*80))
		return 4;
	else
		return 0;
}
...
void display(char* map[][3])
{
	...
	for(i=0;i<3;i++)
	{
		for(j=0;j<3;j++)
		{
			ILI9341_DisplayStringEx(j*80+80,i*80,80,80,map[i][j],0);
		}       
	}			
}
...
void IsWin(char* map[][3])
{
	int i,j;
  int count = 0;
	char* table[3][3]={{"1","2","3"},{"4","5","6"},{"7","8"," "}};
  for(i = 0;i<3;i++)
  {
		for (j=0;j<3;j++)
		{
			if (table[i][j]==map[i][j])
				count++;
		}
	}
  if (count >= 8)
